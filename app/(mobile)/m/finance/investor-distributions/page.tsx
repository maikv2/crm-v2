...
type MonthlyItem = {
  ...
  stockReplenishmentCents?: number | null;
...
export default function MobileFinanceInvestorDistributionsPage() {
  ...
  const summary = useMemo(() => ({
    ...
    stockReplenishment: filtered.reduce((s, i) => s + Number(i.stockReplenishmentCents ?? 0), 0),
  ...
  return (
    <MobilePageFrame title="Repasses investidores" subtitle="Distribuição mensal por região" desktopHref="/investors/distributions">
      <MobileCard>
      ...
      {loadingMonthly ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {statBox("Total investidores (60%)", formatMoneyBR(summary.investorPool), `${summary.regions} região(ões)`, colors.primary)}
            {statBox("Reposição de estoque (30%)", formatMoneyBR(summary.stockReplenishment), "sobre o lucro operacional", "#f59e0b")}
          </div>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {filtered.map((item) => (
                  <div key={item.regionId} style={{
                    ...
                    {/* Total investidores e reposição de estoque */}
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      ...
                      <div style={{ borderRadius: 10, background: colors.isDark ? "#0b1324" : "#fff", border: `1px solid ${colors.border}`, padding: "8px 10px" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#92400e", textTransform: "uppercase" }}>Repos. estoque</div>
                        <div style={{ fontSize: 14, fontWeight: 900, color: "#f59e0b", marginTop: 2 }}>{formatMoneyBR(item.stockReplenishmentCents)}</div>
                      </div>
                    </div>
                  </div>
                ))}
