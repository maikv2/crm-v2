  ...
  return (
    <div style={{ minHeight: "100vh", background: pageBg, padding: 24, color: theme.text }}>
      <div style={{ maxWidth: 900 }}>
        ...
        {/* Card principal: total a receber */}
        ...
        {/* Lista de pedidos */}
        {orders.length > 0 && (
          <div style={{ border: `1px solid ${border}`, borderRadius: 16, background: cardBg, overflow: "hidden" }}>
            ...
            {orders.map((order) => (
              <div
                key={order.orderId}
                style={{ padding: "14px 16px", borderBottom: `1px solid ${border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}
              >
                <div style={{ flex: 1 }}>
                ...
                <div style={{ textAlign: "right", minWidth: 160 }}>
                  {order.commissionThisWeekCents > 0 && (
                    <div style={{ fontWeight: 800, color: order.isCurrentWeekSale ? "#1d4ed8" : "#7e22ce" }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 500, color: muted, marginLeft: 4 }}>a receber</span>
                    </div>
                  )}
                  {order.pendingCents > 0 && (
                    <div style={{ fontSize: 13, color: "#92400e", marginTop: 2 }}>
                      {money(order.pendingCents)}
        ...
        {orders.length === 0 && (
          <div style={{ border: `1px solid ${border}`, borderRadius: 16, padding: 32, background: cardBg, textAlign: "center", color: muted }}>
        ...
        <p style={{ fontSize: 12, color: muted, marginTop: 16 }}>
          ...
          &ldquo;Próximo acerto&rdquo; será pago quando o cliente quitar o pedido.
        </p>
      </div>
    </div>
  );
...
function BlockCard({
  ...
  return (
    <div style={{ border: `1px solid ${border}`, borderRadius: 14, padding: 16, background: bg }}>
