...
export async function GET(
  ...
  try {
    ...
    // ── Mapeia campos fiscais flat do Prisma para o objeto `nfe` esperado pela UI ──
    // O Prisma armazena: nfeStatus, nfeNumber, nfeKey, nfeXmlUrl
    // A página espera:   order.nfe.{ status, number, accessKey, xmlUrl, pdfUrl, issuedAt }
    function mapNfeStatus(
      raw?: string | null
    ): "PROCESSING" | "WAITING" | "ISSUED" | "ERROR" | "REJECTED" | null {
      switch (raw) {
        case "AUTHORIZED":  return "ISSUED";
    ...
    const nfeStatus = mapNfeStatus(order.nfeStatus);
    const nfe =
      nfeStatus !== null || order.nfeNumber || order.nfeKey
        ? {
            status: nfeStatus,
            number: order.nfeNumber ?? null,
            accessKey: order.nfeKey ?? null,
            xmlUrl: order.nfeXmlUrl ?? null,
            // PDF pode ser buscado pelo endpoint /api/orders/[id]/nfe/pdf
            pdfUrl: null as null,
            issuedAt: nfeStatus === "ISSUED" ? order.updatedAt?.toISOString?.() ?? null : null,
          }
        : null;

    return NextResponse.json({ ...order, nfe });
  } catch (error) {
