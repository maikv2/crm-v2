  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <InfoBox
          label="Base a distribuir (70%)"
          value={moneyFromCents(item.distributableCents)}
          theme={theme}
          color="#22c55e"
        />
      </div>
  ...
  const totals = useMemo(() => {
    ...
    return items.reduce(
      (acc, item) => {
        ...
        acc.distributableCents += item.distributableCents;
    ...
    >
      ...
      >
        <div>
          ...
          >
            Lucro operacional do mês, 30% reservado para reposição de estoque e o restante dividido 60% investidor / 40% empresa.
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0, 1fr))", gap: 14, marginBottom: 24 }}>
        ...
        <SummaryCard title="Reposição de estoque (30%)" value={moneyFromCents(totals.stockReplenishmentCents)} theme={theme} color="#f59e0b" />
        <SummaryCard title="Base a distribuir (70%)" value={moneyFromCents(totals.distributableCents)} theme={theme} color="#22c55e" />
        <SummaryCard title="Investidores (60%)" value={moneyFromCents(totals.investorPoolCents)} theme={theme} color="#2563eb" />
        <SummaryCard title="Empresa (40%)" value={moneyFromCents(totals.companyPoolCents)} theme={theme} />
      </div>
