...
export default function MobileRepMapPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!res.ok) {
          throw new Error(json?.error || "Erro ao carregar mapa.");
        }
  ...
  return (
    <MobileRepPageFrame
      title="Mapa"
      subtitle="Clientes e prospectos com rota"
      desktopHref="/rep/map"
    >
      {loading ? (
        <MobileCard>Carregando mapa...</MobileCard>
      ) : error ? (
        ...
        <>
          ...
          <MobileCard>
            <MobileSectionTitle title="Pontos do mapa" />
