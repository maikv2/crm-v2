...
async function main() {
  await prisma.$transaction(async (tx) => {
    ...
    if (existingOrder) {
      throw new Error("Pedido 28 já existe.");
    }

    let client = await tx.client.findUnique({
      where: { cnpj: "79237517000159" },
    });
    ...
    if (!client) {
      client = await tx.client.create({
        data: {
          name: "Hotel Berte",
          tradeName: "Hotel Berte",
          legalName: "Vilson José Berte Me",
          cnpj: "79237517000159",
          whatsapp: "4933580195",
          ...
          state: "SC",
          cep: "89819000",
          stateRegistration: "251286282",
          regionId: REGION_ID,
          active: true,
          roleClient: true,
          mapStatus: "CLIENT",
          notes: "Importado manualmente pelo pedido 28.",
        },
      });
    }
    ...
    const order = await tx.order.create({
      data: {
        ...
        nfeNumber: "504",
        nfeKey: "42260439531220000187550010000005041663044212",
    ...
    await tx.financeTransaction.create({
      data: {
        ...
        paymentStatus: "PENDING",
        paymentReceiver: "MATRIX",
        description: "Venda pedido 28 - Hotel Berte",
        amountCents: 4500,
        regionId: REGION_ID,
        orderId: order.id,
        dueDate: DUE_DATE,
    ...
    console.log("Pedido 28 importado com sucesso");
  });
}
